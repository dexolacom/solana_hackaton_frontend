...
export const useBurnPortfolio = () => {
  ...
  const burnNft = async ({ portfolioId, nftId }: BurnPortfolioArgs) => {
    ...
    const { collectionMint, collectionMetadata, onchainCollectionData } = await getCollectionAddresses(portfolioId);

    const { nftMint, nftMetaData, nftMasterEdition, nftATA, nftRecord } = await getNftAddresses({
