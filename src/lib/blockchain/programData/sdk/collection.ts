...
export function getPortfolioCollectionAddresses(
  ...
  const collection = PublicKey.findProgramAddressSync(
    [Buffer.from('collection'), BISCUIT_PROGRAM.toBuffer(), Buffer.from([collection_id])],
    BISCUIT_PROGRAM
  )[0];

  const metadata = PublicKey.findProgramAddressSync(
    [Buffer.from('metadata'), TOKEN_METADATA_PROGRAM.toBuffer(), collection.toBuffer()],
    TOKEN_METADATA_PROGRAM
  )[0];

  const masterEdition = PublicKey.findProgramAddressSync(
    [Buffer.from('metadata'), TOKEN_METADATA_PROGRAM.toBuffer(), collection.toBuffer(), Buffer.from('edition')],
    TOKEN_METADATA_PROGRAM
  )[0];

  const collectionOnchaindata = PublicKey.findProgramAddressSync(
    [Buffer.from('onchain-data'), collection.toBuffer()],
    BISCUIT_PROGRAM
  )[0];
