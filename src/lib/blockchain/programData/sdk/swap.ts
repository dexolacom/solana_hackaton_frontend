...
export async function getSwapPortfolioData(
  ...
  for (const token of tokens) {
    ...
    const ata = getAssociatedTokenAddressSync(token.key, portfolio, true);
...
export async function getSwapData(
  ...
  console.log(
    `Token A: ${token_a.toBase58()}. Token B: ${token_b.toBase58()}. Amount: ${amount.toString()}. A to B: ${a_to_b}.`
  );
