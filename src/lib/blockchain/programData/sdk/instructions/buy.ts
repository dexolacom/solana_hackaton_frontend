...
export function getBuyPortfolioInstruction(
  ...
  const fundingAccount = getAssociatedTokenAddressSync(payment_token, user, false);

  const portfolioPaymentAccount = getAssociatedTokenAddressSync(payment_token, portfolio.mint, true);

  const treasury_ata_payment_token = getAssociatedTokenAddressSync(payment_token, treasury, true);

  return program.methods
    .buyPortfolio(portfolio_id, collection_id, uri, amount)
    .accounts({
      payer: user,
