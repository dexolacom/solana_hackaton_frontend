...
export function getPortfolioAddresses(
  ...
  const mint = PublicKey.findProgramAddressSync(
    [Buffer.from('token'), collection_mint.toBuffer(), Buffer.from([portfolio_id])],
    BISCUIT_PROGRAM
  )[0];
  ...
  const portfolioData = PublicKey.findProgramAddressSync(
    [Buffer.from('onchain-data'), mint.toBuffer()],
    BISCUIT_PROGRAM
  )[0];

  const userPortfolioAta = getAssociatedTokenAddressSync(mint, user_pubkey, false);
...
export function getCreatePortfolioTokenAccountsInstructions(
  portfolio: PublicKey,
  ...
  const result = [];
  for (const token of collection_tokens) {
    const ata = getAssociatedTokenAddressSync(token, portfolio, true);

    result.push(createAssociatedTokenAccountInstruction(user, ata, portfolio, token));
  }
  return result;
}
